...
public class CdoSession implements CdoSessionService, ManagedService {
	...
	public static final String CACTOOPTUSER = "cactoScaleUser";
	public static final String CACTOOPTPASSWORD = "cactoScalePassword";
	...
	@Override
	public synchronized CactosCdoSession getCactosCdoSession(CactosUser cactosUser) {
		if(cactosUser == null) return null;
		createRefreshOrRetrieveSession(cactosUser);
		String userData = "";
		if(null != cactosUser) userData = "for user " + cactosUser;
		openSessions.get(cactosUser).printOpenConnections(userData);
		return openSessions.get(cactosUser);
	}

	private void createRefreshOrRetrieveSession(CactosUser cactosUser) {
	...
	private CactosCdoSession createCactosCdoSession(CactosUser cactosUser) {
		log.info("create new CactosCdoSession for user " + cactosUser);
		IPasswordCredentialsProvider provider = getCredentialsFor(cactosUser);
		String repoName = (String) this.properties.get(CdoSession.CDOREPO);
		String endpoint = (String) this.properties.get(CdoSession.CDOENDPOINT);
				
		try {
			CactosCdoSession cactosCdoSession = CactosCdoSession.createSession(provider, repoName, endpoint, this.properties); 
